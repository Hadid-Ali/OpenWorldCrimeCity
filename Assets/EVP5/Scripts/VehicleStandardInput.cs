...
public class VehicleStandardInput : MonoBehaviour
	...
	void Update ()
        ...
        public float motorInput, steerInput = 0;
        public Vector2 controlsSenstivity = new Vector2(1, 1);
    void FixedUpdate ()
		{
		if (target == null) return;
		...
		if (continuousForwardAndReverse)
            ...
            target.steerInput = GameplayHUD.isRight ? 1f : GameplayHUD.isLeft ? -1f : 0f;

		target.throttleInput = GameplayHUD.isAccelerate ? 1f : GameplayHUD.isBrake ? -1f : 0f;
