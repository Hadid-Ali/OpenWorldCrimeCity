...
public class VehicleController : EnterableVehicle
	{
	...
	VehicleFrame m_vehicleFrame;
        ...
        {
            if (this.isUberVehicle)
            {
                this.isPlayerVehicle = true;
                GameManager.instance.gameplayHUD.ShowMessage("Do You Want to Earn by Driving?", "Earn Cash", MessageMode.YesNo, GameManager.instance.taxiModule.RideOfferAccepted, GameManager.instance.taxiModule.RideOfferDeclined);
            }
        }
	...
	float FixSteerAngle (WheelData wd, float inputSteerAngle)
 	...
 	void UpdateSuspension (WheelData wd)
		...
		if (wd.grounded && !disableWheelHitCorrection)
			...
			#else
			bool collided = Physics.Raycast(wd.origin, -wd.transform.up, out rayHit, wd.collider.suspensionDistance + wd.collider.radius, Physics.DefaultRaycastLayers);
			#endif
	...
	public void OnDrawGizmos ()
		{
            ...
            Gizmos.DrawSphere(this.transform.position, 20f);
		if (!enabled) return;
