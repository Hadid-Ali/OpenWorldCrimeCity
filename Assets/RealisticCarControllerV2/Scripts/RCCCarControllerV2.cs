...
public class RCCCarControllerV2 : EnterableVehicle {
    ...
    public override void OnVehicleEntered()
    {
        Debug.Log("Vehicle Entered");
    }

    public override void OnVehicleExit()
    {
	...
	public void CreateWheelColliders (){
		...
		foreach(Transform wheel in allWheelTransforms){
            ...
            wc.radius = (wheel.GetComponent<MeshRenderer>().bounds.size.y / 2f) / transform.localScale.y;
            ...
            wc.forceAppPointDistance = .25f;
            wc.mass = 100f;
            wc.wheelDampingRate = .5f;

			wheelcollider.transform.localPosition = new Vector3(wheelcollider.transform.localPosition.x, wheelcollider.transform.localPosition.y + (wc.suspensionDistance / 2f), wheelcollider.transform.localPosition.z);
...
//		_wheelParticles[3].transform.localPosition = Vector3.zero;
    ...
    private  Vector2 controlsSenstivity = new Vector2(1f, 5f);

	public void KeyboardControlling (){

        //Motor Input.

        //if(Input.GetKey(KeyCode.UpArrow) | GameplayHUD.isAccelerate)
        //{
        //    this.motorInput += Time.deltaTime*this.controlsSenstivity.x;
        //    this.motorInput = Mathf.Clamp(this.motorInput, 0, 1);
        //}
        //else if(Input.GetKey(KeyCode.DownArrow) | GameplayHUD.isBrake)
        //{
        //    this.motorInput -= Time.deltaTime * this.controlsSenstivity.x;
        //    this.motorInput = Mathf.Clamp(this.motorInput, -1, 1);
        //}
        //else
        //{
        //   motorInput = Mathf.Lerp(this.motorInput,0,Time.deltaTime * this.controlsSenstivity.x);
        //}

        //if (Input.GetKey(KeyCode.RightArrow) | GameplayHUD.isRight)
        //{
        //    this.steerInput += Time.deltaTime * this.controlsSenstivity.y/4;
        //    this.steerInput = Mathf.Clamp(this.steerInput, 0, 1);
        //}
        //else if (Input.GetKey(KeyCode.LeftArrow) | GameplayHUD.isLeft)
        //{
        //    this.steerInput -= Time.deltaTime*this.controlsSenstivity.y/4;
        //    this.steerInput = Mathf.Clamp(this.steerInput, -1, 0);
        //}
        //else
        //{
        //    steerInput = Mathf.Lerp(this.steerInput, 0, Time.deltaTime* this.controlsSenstivity.y);
        //}

        //  if (!changingGear)
        //      motorInput = Mathf.Clamp(CnInputManager.GetAxis("Accelerate"), 0f, 1f);
        //  else
        //      motorInput = Mathf.Clamp(CnInputManager.GetAxis("Decelerate"), -1f, 0f);

        ////  Steering Input.
        //  if (Mathf.Abs(Input.GetAxis("Horizontal")) > .05f)
        //      steerInput = Mathf.Lerp(steerInput, CnInputManager.GetAxis("Horizontal"), Time.deltaTime * 10);
        //  else
        //      steerInput = Mathf.Lerp(steerInput, CnInputManager.GetAxis("Horizontal"), Time.deltaTime * 10);

        motorInput = GameplayHUD.isAccelerate ? 1f : GameplayHUD.isBrake ? -1f : 0f;

        if (GameplayHUD.isRight)
        {
            steerInput += Time.deltaTime * 2f;
        }
        else if (GameplayHUD.isLeft)
        {
            steerInput -= Time.deltaTime * 2f;
        }
        else
