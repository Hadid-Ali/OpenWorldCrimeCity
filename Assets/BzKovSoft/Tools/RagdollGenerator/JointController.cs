...
namespace BzKovSoft.Tools.RagdollGenerator
{
	...
	public static class JointController
	{
		...
		//	Handles.ArrowCap(0, position, Quaternion.LookRotation(axisDir), size * 1.1f);
		...
		//	Handles.ArrowCap(0, position, Quaternion.LookRotation(swingAxisDir), size * 1.1f);
		...
		//	Handles.ArrowCap(0, position, Quaternion.LookRotation(direction), size * 2f);
		...
		static float ProcessLimit(Vector3 position, Vector3 planeNormal, Vector3 startDir, float size, float limit)
		{
			...
			bool positionChanged = true;// Handles.FreeMoveHandle(controllerPos, Quaternion.identity, size * 0.1f, Vector3.zero, Handles.) != controllerPos;
			if (positionChanged)
			{
