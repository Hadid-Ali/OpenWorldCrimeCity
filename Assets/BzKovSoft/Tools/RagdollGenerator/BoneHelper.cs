...
namespace BzKovSoft.Tools.RagdollGenerator
{
	...
	public sealed class BoneFixer : Editor
	{
		...
		//	Handles.ArrowCap(1, _go.transform.position, Quaternion.LookRotation(playerDirection, Vector3.up), size);
		...
		void DrawControls()
		{
			for (int i = 0; i < _transforms.Length; i++)
			{
				...
				//if (Handles.Button(pos, Quaternion.identity, size / 6f, size / 6f, Handles.SphereCap))
				//	_curPointIndex = i;
		...
		void ProcessColliderMove(Quaternion rotatorRotation, Transform transform, Vector3 pos)
		{
			...
			if (cCollider != null)
			{
				...
				//cCollider.radius = Handles.ScaleValueHandle(cCollider.radius, pos, t, cCollider.radius * magicNumber, Handles.CircleCap, 0);
				...
				//cCollider.height = Handles.ScaleValueHandle(cCollider.height, scaleHeightPos1, t, size * 0.5f, Handles.cap, 0);
				//cCollider.height = Handles.ScaleValueHandle(cCollider.height, scaleHeightPos2, t, size * 0.5f, Handles.DotCap, 0);
